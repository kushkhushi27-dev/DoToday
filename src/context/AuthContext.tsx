    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      if (currentUser) {
        ...
        try {
          ...
          // Fail silently with console.warn (don't block the user)
          console.warn('Failed to auto-sync user profile to Firestore:', syncError);
        }
      }

      setUser(currentUser);
      // loading starts as true, set to false after first onAuthStateChanged callback
      setLoading(false);
    });
    ...
    try {
      ...
      switch (err?.code) {
        ...
        case 'auth/cancelled-popup-request':
          friendlyMessage = 'Multiple open attempts';
          break;
